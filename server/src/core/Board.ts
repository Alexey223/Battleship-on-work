...
export const placeShipOnBoardCells = (board: Board, ship: Ship): Board => {
  const newBoard = {
    ...board,
    cells: board.cells.map(row => row.map(cell => ({ ...cell }))),
    // ships: [...board.ships, ship] // Удаляем эту строку, массив ships уже установлен
  };
  ...
  for (let i = 0; i < ship.size; i++) {
    ...
    if (newBoard.cells[y]?.[x]) { 
        newBoard.cells[y][x].state = 'ship';
        newBoard.cells[y][x].shipId = ship.id;
        newBoard.cells[y][x].shipColor = ship.color;
    } else {
        console.error(`[placeShipOnBoardCells] Cell (${x},${y}) is out of bounds for ship ${ship.id}. Board size: ${newBoard.size}`);
    }
  }
  ...
  if (targetCell.state === 'ship' && targetCell.shipId !== undefined) {
    ...
    if (shipIndex !== -1) {
      ...
      if (ship.hits >= ship.size) {
        ship.isSunk = true;
        // Меняем состояние всех ячеек потопленного корабля
        for (let i = 0; i < ship.size; i++) {
          const currentShipCellX = ship.direction === 'horizontal' ? ship.x + i : ship.x;
          const currentShipCellY = ship.direction === 'vertical' ? ship.y + i : ship.y;
          if (newBoard.cells[currentShipCellY]?.[currentShipCellX]) {
            newBoard.cells[currentShipCellY][currentShipCellX].state = 'sunk_ship_segment';
          }
        }
        return { board: newBoard, hit: true, sunkShipObject: { ...ship } };
      } else {
        // Если просто попадание, то только одна ячейка становится 'hit'
        targetCell.state = 'hit'; // Это уже было сделано выше
      }
