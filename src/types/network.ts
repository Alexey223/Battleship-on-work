import { Board, GameState, Ship } from './game'; // Предполагаем, что game.ts будет доступен или типы будут скопированы/синхронизированы
...
export interface PlaceShipsClientMessage {
  ...
  payload: {
    ships: Ship[]; // Полная конфигурация кораблей игрока
...
export interface GameStartServerMessage {
  ...
  payload: {
    initialGameState: Partial<GameState>; // Начальное состояние игры (доски, текущий игрок)
...
export interface GameUpdateServerMessage {
  ...
  payload: {
    updatedBoard: Board; // Обновленная доска игрока, по которому стреляли
...
export interface ShotResultServerMessage {
  ...
  payload: {
    ...
    updatedTargetBoard: Board; // Обновленная доска атакованного игрока
    nextPlayerIndex: 0 | 1;    // Чей ход следующий
    sunkShip?: Ship;             // Информация о потопленном корабле, если есть
    // shipId?: number; // ID потопленного корабля, если есть (можно заменить на sunkShip)
    // onBoardOfPlayerIndex: 0 | 1; // Заменено на targetPlayerIndex
    // isSunkShipMarked?: boolean; // Детали потопления можно включить в updatedTargetBoard или sunkShip
    isGameOver: boolean;
